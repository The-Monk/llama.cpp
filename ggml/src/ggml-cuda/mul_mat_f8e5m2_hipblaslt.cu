// See mul_mat_f8e5m2_hipblaslt.cuh for the full rationale.
// F8E5M2 prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY F8E5M2-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_f8e5m2_hipblaslt.cuh"
...
constexpr int   QKK   = QK_F8E5M2;           // 32 elements per F8E5M2 block
...
__device__ __forceinline__ float f5k_half2float(ggml_half h) {
    return __half2float(*reinterpret_cast<const __half *>(&h));
}

// MEASURED 2026-08-22 (sparse-llama-8B requant to F8E5M2, R9700, pp512/pp2048):
// the route WINS. Interleaved 3-round A/B at pp512: OFF 4011.82/4046.13/4067.84
// vs ON 4733.90/4716.40/4737.16 (+17%, every round). pp2048: 4776.77 -> 5553.24
// (+16%). fp8/e4m3 mode ties int8 mode (4551.92 / 5557.12). Weight-cache budget
// sweep at pp512, all at or above the mmq baseline of 4026.25:
//   WCACHE_MB 0 -> 4062.80 (route inert, falls back to mmq), 1000 -> 4194.65,
//   4000 -> 4723.06, 12000 (default) -> 4576.38.
// Quality is neutral (Bonsai-1.7B-F8E5M2 PPL 25.5208 OFF -> 25.4259 ON).
//
// CORRECTION: an earlier note here claimed this route was a -43% REGRESSION
// (3908.99 -> 2232.50). That measurement was wrong -- it came from a SEQUENTIAL
// A/B (OFF first, ON second) run straight after a 16 GB llama-quantize and a
// bench sweep, i.e. ON was measured on a heat-soaked card. It does not
// reproduce at any cache budget. Perf claims on this box need interleaved
// rounds; back-to-back blocks pick up thermal drift and ordering effects.
// F8E5M2 dequant: 32 signed OCP e5m2 (bf8) elements x one fp16 per-block scale d.
//   w = e5m2_to_fp32(qs[t]) * d,  d = half2float(blk->d).
// NOTE: only the SOURCE decode is e5m2. In MODE_F8 the operands are re-encoded
// to e4m3 (ggml_cuda_fp32_to_e4m3 / HIP_R_8F_E4M3) -- that is the format
// hipBLASLt computes in on RDNA4, same as every other fp8-mode route here.
__device__ __forceinline__ float f5k_dequant_elem(const block_f8e5m2 * blk, int t, float d) {
    return ggml_cuda_e5m2_to_fp32(blk->qs[t]) * d;
}

// Requant a F8E5M2 weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_f8e5m2_to_int8_perchannel(
    ...
    const block_f8e5m2 * row = (const block_f8e5m2 *)(wdata + n * nb01);
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float   d    = f5k_half2float(row[b].d);
        const float   w    = f5k_dequant_elem(&row[b], t, d);
...
__global__ void k_requant_f8e5m2_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float d = f5k_half2float(row[b].d);
        amax = fmaxf(amax, fabsf(f5k_dequant_elem(&row[b], t, d)));
    }
    __shared__ float sred[1024];
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        const int64_t b = l / QKK; const int t = (int)(l % QKK);
        const float d = f5k_half2float(row[b].d);
        out[l] = ggml_cuda_fp32_to_e4m3(f5k_dequant_elem(&row[b], t, d) * inv);
    }
}
...
// independent of the source quant, so F8E5M2 and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_F8E5M2_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_F8E5M2_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_F8E5M2_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_f8e5m2_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_f8e5m2_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_f8e5m2_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    if (src0->type != GGML_TYPE_F8E5M2)                       return false;
    ...
    static const int64_t M_THRESH = [](){
        const char * e = getenv("GGML_HIP_F8E5M2_HIPBLASLT_MTHRESH");
...
bool ggml_cuda_op_mul_mat_f8e5m2_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_F8E5M2);
    ...
    static const int mode = (getenv("GGML_HIP_F8E5M2_HIPBLASLT_FP8") != nullptr) ? MODE_F8 : MODE_I8;
    ...
    // route this file was ported from, NOT on F8E5M2; the mechanism is
    // shared, the magnitudes are unverified here.)
    // Return false -> caller falls through to mmq. Set
    // GGML_HIP_F8E5M2_HIPBLASLT_FORCE=1 to restore the old always-route
    ...
    static const bool route_force = (getenv("GGML_HIP_F8E5M2_HIPBLASLT_FORCE") != nullptr);
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_f8e5m2_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_f8e5m2_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_f8e5m2_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_f8e5m2_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
