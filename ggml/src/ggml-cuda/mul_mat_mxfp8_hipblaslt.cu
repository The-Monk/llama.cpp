// See mul_mat_mxfp8_hipblaslt.cuh for the full rationale.
// MXFP8 prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY MXFP8-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_mxfp8_hipblaslt.cuh"
...
constexpr int   QKK   = QK_MXFP8;           // 32 elements per MXFP8 block
...
// MXFP8 dequant: 32 OCP e4m3 elements x one UE8M0 shared block scale.
//   w = e4m3_to_fp32(qs[t]) * d,  where d = e8m0_to_fp32(blk->e) (power-of-2).
// Matches dequantize_mxfp8 (dequantize.cuh:140).
__device__ __forceinline__ float mxk_dequant_elem(const block_mxfp8 * blk, int t, float d) {
    return ggml_cuda_e4m3_to_fp32(blk->qs[t]) * d;
}

// Requant a MXFP8 weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_mxfp8_to_int8_perchannel(
    ...
    const block_mxfp8 * row = (const block_mxfp8 *)(wdata + n * nb01);
...
__global__ void k_requant_mxfp8_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
...
// independent of the source quant, so MXFP8 and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_MXFP8_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_MXFP8_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_MXFP8_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_mxfp8_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_mxfp8_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_mxfp8_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    if (src0->type != GGML_TYPE_MXFP8)                       return false;
    ...
    static const int64_t M_THRESH = [](){
        const char * e = getenv("GGML_HIP_MXFP8_HIPBLASLT_MTHRESH");
...
bool ggml_cuda_op_mul_mat_mxfp8_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_MXFP8);
    ...
    static const int mode = (getenv("GGML_HIP_MXFP8_HIPBLASLT_FP8") != nullptr) ? MODE_F8 : MODE_I8;
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_mxfp8_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_mxfp8_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_mxfp8_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_mxfp8_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
