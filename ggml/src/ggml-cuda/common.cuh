...
// Signed E4M3 (OCP e4m3fn): 1 sign, 4 exp bits (bias=7), 3 mantissa bits.
// Used for GGML_TYPE_F8E4M3 weight values (Path X). Portable software decode
// (no hardware fp8 instruction) -- Phase 1a is dequant-only, no WMMA.
static __device__ __forceinline__ float ggml_cuda_e4m3_to_fp32(uint8_t x) {
    ...
    if (exp == 0) {
        ...
        raw = ldexpf(1.0f + (float) man / 8.0f, exp - 7); // NaN pattern (exp=15,man=7) decodes as a large finite value here; never produced by our own quantizer
    }
    return sign ? -raw : raw;
}
...
// ROC8: MXFP6's E3M2 (3 exp bits, bias=3, 2 mantissa bits, OCP MX FP6 -- no
// Inf/NaN encoding, all 64 codes finite) -> e4m3fn LOSSLESS bit-level
// upconvert. E3M2's value set is a strict subset of e4m3's (e4m3 has 4 more
// exponent steps of headroom either side and one extra mantissa bit), so
// every finite E3M2 value round-trips EXACTLY -- this is what lets the
// existing fp8 dp4a/hardware-dot2 compute path (ggml_cuda_dot2_e4m3_q8 above)
// be reused verbatim for MXFP6 decode, no new compute kernel needed. Normal
...
// Mirrors ggml_e3m2_to_e4m3 (ggml-impl.h) bit-for-bit -- keep the two in sync.
static __device__ __forceinline__ uint8_t ggml_cuda_e3m2_to_e4m3(uint8_t code) {
    const uint32_t sign = (code >> 5) & 1;
    const uint32_t exp3 = (code >> 2) & 0x7;
    const uint32_t man2 = code & 0x3;

    if (exp3 == 0) {
        if (man2 == 0) {
            return (uint8_t) (sign << 7); // +-0
        }
        const uint32_t exp4 = (man2 == 1) ? 3 : 4;
        const uint32_t man3 = (man2 == 3) ? 4 : 0;
        return (uint8_t) ((sign << 7) | (exp4 << 3) | man3);
    }

    const uint32_t exp4 = exp3 + 4;  // bias shift: (exp3-3)+7 = exp3+4
    const uint32_t man3 = man2 << 1; // 2-bit mantissa -> 3-bit (append 0 LSB)
    return (uint8_t) ((sign << 7) | (exp4 << 3) | man3);
}

// ROC9 v3: 64-entry E3M2->e4m3 LUT, precomputed at BUILD time (host-side,
// mirrors ggml_e3m2_to_e4m3 bit-for-bit -- generated by running that exact
// function over all 64 codes, not hand-derived) to replace the ~17
// instructions/element (branch-predicated bias-shift + subnormal-renorm,
// ISA-confirmed via disassembly: ~254 v_cndmask/v_cmp + ~300 shift/or in
// the decode kernel) with a single lookup. `static __constant__` per the
// task ask -- tried first because a 64-BYTE table is small enough to want
// broadcast/constant-cache residency; NOTE the access pattern here is
// NOT uniform-across-lanes (each of the 32 threads in a wave decodes a
// DIFFERENT weight row -> a DIFFERENT `code` -> a DIFFERENT lut[] index in
// the same instruction), so this is exactly the divergent-index case
// constant memory is NOT optimized for (that's the uniform-broadcast case);
// measured, not assumed -- see the mxfp6 v3 bench note in vecdotq.cuh.
static __constant__ uint8_t mxfp6_e3m2_to_e4m3_lut[64] = {
    0x00, 0x18, 0x20, 0x24, 0x28, 0x2a, 0x2c, 0x2e,
    0x30, 0x32, 0x34, 0x36, 0x38, 0x3a, 0x3c, 0x3e,
    0x40, 0x42, 0x44, 0x46, 0x48, 0x4a, 0x4c, 0x4e,
    0x50, 0x52, 0x54, 0x56, 0x58, 0x5a, 0x5c, 0x5e,
    0x80, 0x98, 0xa0, 0xa4, 0xa8, 0xaa, 0xac, 0xae,
    0xb0, 0xb2, 0xb4, 0xb6, 0xb8, 0xba, 0xbc, 0xbe,
    0xc0, 0xc2, 0xc4, 0xc6, 0xc8, 0xca, 0xcc, 0xce,
    0xd0, 0xd2, 0xd4, 0xd6, 0xd8, 0xda, 0xdc, 0xde,
};

// ROC8: unpack 4 six-bit E3M2 codes from a block_mxfp6 group's 3 packed
// bytes (see block_mxfp6, ggml-common.h, for the bit layout -- CPU
// (ggml-quants.c) and GPU independently implement this same layout, kept in
// sync by construction/comment cross-reference).
static __device__ __forceinline__ void mxfp6_unpack4(const uint8_t * qs3, uint8_t out[4]) {
    const uint8_t b0 = qs3[0];
    const uint8_t b1 = qs3[1];
    const uint8_t b2 = qs3[2];
    out[0] = (uint8_t) (b0 & 0x3F);
    out[1] = (uint8_t) (((b0 >> 6) & 0x03) | ((b1 & 0x0F) << 2));
    out[2] = (uint8_t) (((b1 >> 4) & 0x0F) | ((b2 & 0x03) << 4));
    out[3] = (uint8_t) ((b2 >> 2) & 0x3F);
}

// ROC9 v2 (mxfp6 decode speed fix): CK-style (AMD Composable Kernel
// pk_float6_e3m2_t::unpack, verified against ck-ref/include/ck_tile/core/
// numeric/pk_f6.hpp) whole-BLOCK bit-offset unpack. v1 (removed:
// get_mxfp6_e4m3x4 called mxfp6_unpack4 on 3 fresh bytes read via qs+3*i
// pointer arithmetic PER GROUP OF 4 -- 8 small, scattered, independently-
// addressed loads for one 32-value block) measured -41% decode vs FP8-E4M3
// (should have been a bandwidth WIN at 6.25 vs 8.25 bpw; wasn't). This
// loads the SAME 24-byte body as 6 pre-composed 32-bit words ONCE per
// vec_dot call, then every one of the 32 element extractions is a pure
// shift+mask against already-loaded registers -- bit_offset = i*6,
// cross-word case handled exactly as CK's unpack() (word_idx = bit_offset
// / 32, bit_in_word = bit_offset % 32, spans two words iff bit_in_word >
// 26). No new global/local memory touched after the initial 6-word load,
// vs v1's repeated small reads.
struct mxfp6_block_words {
    uint32_t w[6]; // 24 bytes = 32 * 6 bits, exactly QK_MXFP6 elements
};

static __device__ __forceinline__ mxfp6_block_words mxfp6_load_block(const uint8_t * qs) {
    mxfp6_block_words b;
#pragma unroll
    for (int k = 0; k < 6; ++k) {
        // Safe byte composition (mirrors get_int_b1): block_mxfp6::qs is
        // NOT guaranteed 4-byte aligned -- it follows a 1-byte e8m0 scale
        // field, so a raw uint32 reinterpret-cast would be unaligned UB.
        b.w[k] = (uint32_t) qs[4*k+0]         | ((uint32_t) qs[4*k+1] << 8) |
                 ((uint32_t) qs[4*k+2] << 16) | ((uint32_t) qs[4*k+3] << 24);
    }
    return b;
}

static __device__ __forceinline__ uint8_t mxfp6_get_code(const mxfp6_block_words & b, int i) {
    const int bit_offset  = i * 6;
    const int word_idx    = bit_offset >> 5;  // / 32
    const int bit_in_word = bit_offset & 31;  // % 32

    if (bit_in_word <= 26) {
        // Value fits entirely in one word.
        return (uint8_t) ((b.w[word_idx] >> bit_in_word) & 0x3F);
    }
    // Value spans two words.
    const int bits_in_first  = 32 - bit_in_word;
    const int bits_in_second = 6 - bits_in_first;
    const uint8_t low_bits  = (uint8_t) ((b.w[word_idx] >> bit_in_word) & ((1u << bits_in_first) - 1));
    const uint8_t high_bits = (uint8_t) ((b.w[word_idx + 1] & ((1u << bits_in_second) - 1)) << bits_in_first);
    return low_bits | high_bits;
}

// Drop-in replacement for the old get_mxfp6_e4m3x4(qs, i): same "packed
// int32 of 4 e4m3 bytes" contract (group index i selects elements
// [4*i, 4*i+3]), but reads from the already-loaded block_words registers
// instead of re-touching memory per call.
static __device__ __forceinline__ int mxfp6_e4m3x4_from_block(const mxfp6_block_words & b, int i) {
    uint32_t packed = 0;
#pragma unroll
    for (int j = 0; j < 4; ++j) {
        const uint8_t code = mxfp6_get_code(b, 4*i + j);
        packed |= ((uint32_t) mxfp6_e3m2_to_e4m3_lut[code]) << (8*j);
    }
    return (int) packed;
}
