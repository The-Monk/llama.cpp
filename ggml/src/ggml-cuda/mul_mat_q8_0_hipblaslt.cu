// See mul_mat_q8_0_hipblaslt.cuh for the full rationale.
// Q8_0 prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY Q8_0-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_q8_0_hipblaslt.cuh"
...
constexpr int   QKK   = QK8_0;           // 32 elements per Q8_0 block
...
__device__ __forceinline__ float q8k_half2float(ggml_half h) {
    return __half2float(*reinterpret_cast<const __half *>(&h));
}

// Q8_0 dequant: block is 32 signed-int8 quants x one fp16 per-block scale d.
//   w = qs[t] * d   (symmetric, no min). d directly accessible (bq8_0->d).
__device__ __forceinline__ float q8k_dequant_elem(const block_q8_0 * blk, int t, float d) {
    return (float)blk->qs[t] * d;
}

// Requant a Q8_0 weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_q8_0_to_int8_perchannel(
    ...
    const block_q8_0 * row = (const block_q8_0 *)(wdata + n * nb01);
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float   d    = q8k_half2float(row[b].d);
        const float   w    = q8k_dequant_elem(&row[b], t, d);
...
__global__ void k_requant_q8_0_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float d = q8k_half2float(row[b].d);
        amax = fmaxf(amax, fabsf(q8k_dequant_elem(&row[b], t, d)));
    }
    __shared__ float sred[1024];
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        const int64_t b = l / QKK; const int t = (int)(l % QKK);
        const float d = q8k_half2float(row[b].d);
        out[l] = ggml_cuda_fp32_to_e4m3(q8k_dequant_elem(&row[b], t, d) * inv);
    }
}
...
// independent of the source quant, so Q8_0 and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_Q8_0_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_Q8_0_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_Q8_0_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_q8_0_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_q8_0_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_q8_0_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    ...
    static const int64_t M_THRESH = [](){
        const char * e = getenv("GGML_HIP_Q8_0_HIPBLASLT_MTHRESH");
...
bool ggml_cuda_op_mul_mat_q8_0_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_q8_0_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_q8_0_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_q8_0_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_q8_0_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
