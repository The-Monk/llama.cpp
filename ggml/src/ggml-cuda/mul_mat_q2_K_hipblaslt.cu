// See mul_mat_q2_K_hipblaslt.cuh for the full rationale.
// Q2_K prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY Q2_K-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_q2_K_hipblaslt.cuh"
...
constexpr int   QKK   = QK_K;           // 256 elements per Q2_K superblock
...
__device__ __forceinline__ float q2k_half2float(ggml_half h) {
    return __half2float(*reinterpret_cast<const __half *>(&h));
}

// Extract (d, dmin) from a Q2_K superblock. In the CUDA build the d/dmin fields
// are nested in the union macro; the accessible member is `dm` (ggml_half2,
// .x=d scale, .y=dmin min-scale) -- same access the decode path uses.
__device__ __forceinline__ float2 q2k_d_dmin(const block_q2_K * blk) {
    return __half22float2(*reinterpret_cast<const __half2 *>(&blk->dm));
}

// MEASURED 2026-08-22 (sparse-llama-8B-Q2_K, R9700, pp512 r=3): 312.11 +/- 3.32
// OFF -> 4783.69 +/- 633.39 ON (15.3x). Read that as "the route bypasses a
// pathological path", NOT as hipBLASLt being 15x better than mmq: 312 t/s
// prefill for an 8B is far below this card's tier (an 8B sparse24-q8 measures
// ~4665 pp512, a 27B Q4_K_M ~991), so upstream Q2_K prefill is the anomaly and
// 4783 is simply the normal 8B tier. The underlying Q2_K prefill path is worth
// fixing on its own account.
// Fully dequantize weight at position t (0..255) within a Q2_K superblock:
//   w = d*sc*q - dmin*mn   (min folded in -> the returned value is the true weight)
// Layout (matches dequantize_block_q2_K): half n=t/128, l=t%32, field g=(t%128)/32,
// scale index is=8n + l/16 + 2g, 2-bit quant from qs[32n + l] at bit offset 2g.
// Q2_K packs scale and min as two 4-bit nibbles in scales[is] (no 6-bit split,
// so there is no get_scale_min_k4 equivalent here).
__device__ __forceinline__ float q2k_dequant_elem(const block_q2_K * blk, int t, float d, float dmin) {
    const int n  = t >> 7;                 // 0..1   (128-element half)
    const int r  = t & 127;                // 0..127 within half
    const int l  = r & 31;                 // 0..31
    const int g  = r >> 5;                 // 0..3   (which 2-bit field)
    const int is = 8*n + (l >> 4) + 2*g;   // 0..15  scale/min index
    const uint8_t qb = blk->qs[32*n + l];
    const int     q  = (qb >> (2*g)) & 3;
    const uint8_t sc = blk->scales[is] & 0xF;
    const uint8_t mn = blk->scales[is] >>  4;
    return d * (float)sc * (float)q - dmin * (float)mn;
}

// Requant a Q2_K weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_q2_K_to_int8_perchannel(
    ...
    const block_q2_K * row = (const block_q2_K *)(wdata + n * nb01);
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float2  dd   = q2k_d_dmin(&row[b]);
        const float   w    = q2k_dequant_elem(&row[b], t, dd.x, dd.y);
...
__global__ void k_requant_q2_K_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float2 dd = q2k_d_dmin(&row[b]);
        amax = fmaxf(amax, fabsf(q2k_dequant_elem(&row[b], t, dd.x, dd.y)));
    }
    __shared__ float sred[1024];
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        const int64_t b = l / QKK; const int t = (int)(l % QKK);
        const float2 dd = q2k_d_dmin(&row[b]);
        out[l] = ggml_cuda_fp32_to_e4m3(q2k_dequant_elem(&row[b], t, dd.x, dd.y) * inv);
    }
}
...
// independent of the source quant, so Q2_K and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_Q2_K_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_Q2_K_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_Q2_K_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_q2_K_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_q2_K_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_q2_K_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    ...
    static const int64_t M_THRESH = [](){
        // THRESHOLD NOTE (2026-08-22, Q2_K route). The 384 default is INHERITED
        // UNMEASURED from the Q4_K route -- it has NOT been swept for Q2_K. The
        // Q4_K sweep it came from found the sign of the win is model-dependent at
        // M=128 (Devstral-24B-Q4_K_M -27%, sparse-llama-8B-Q4_K +8.3%, +30.9% at
        // M=256), so 384 was chosen there to never regress. Q2_K unpack is cheaper
        // per element than Q4_K (2-bit quants, nibble scale/min, no 6-bit split),
        // so the true Q2_K crossover is plausibly LOWER -- sweep before trusting
        // this number, and override per model with the env var below.
        // QUALITY NOTE (2026-08-22, measured, ch10 wikitext-2, int8 mode): the
        // per-row int8 collapse costs a MODEL-DEPENDENT amount on Q2_K, and the
        // spread is much wider than the Q4_K route sees:
        //   sparse-llama-8B-Q2_K   14.6427 -> 14.5920  (-0.35%, noise)
        //   Qwen3-8B-Q2_K          18.1584 -> 19.2633  (+6.1%)
        // Same-model Q4_K route for scale: 9.1316 -> 9.1396 (+0.09%). Squeezing
        // Q2_K per-16 scales AND mins into one scale per output row is a harsher
        // range collapse than Q4_K's, so PPL-check per model before enabling.
        const char * e = getenv("GGML_HIP_Q2_K_HIPBLASLT_MTHRESH");
...
bool ggml_cuda_op_mul_mat_q2_K_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_Q2_K);
    ...
    static const int mode = (getenv("GGML_HIP_Q2_K_HIPBLASLT_FP8") != nullptr) ? MODE_F8 : MODE_I8;
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_q2_K_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_q2_K_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_q2_K_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_q2_K_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
