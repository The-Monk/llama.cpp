...
// GGML_HIP_DEDUP_MMVQ_QUANT (default: unset/OFF): dedup the activation
// quantize dispatch across sibling mmvq matmuls that share one input tensor
// (wq/wk/wv, ffn_gate/ffn_up, wqkv/wqkv_gate within one decoder layer). The
// cache lives on the backend context and is reset every graph_compute
// (ggml-cuda.cu), so an entry can never survive a graph rebuild. Lossless --
// the cached bytes are the bytes the skipped launch would have produced.
// _BATCH widens it to ne11>1 (spec-decode verify); _FP8 extends it to the
// F8E4M3 native-e4m3 activation quantizer.
static bool ggml_cuda_dedup_mmvq_quant_enabled() {
    // Default ON since 2026-08-11: measured +2.0 t/s (+3.1%) on Bonsai-27B
    // Q1_0 decode, 1x R9700 (interleaved A/B, 64.27 -> 66.25), bit-identical
    // by construction (reuses the identical quantized activation instead of
    // recomputing it). GGML_HIP_DEDUP_MMVQ_QUANT_DISABLE turns it off; the
    // old opt-in var is still accepted and now redundant. NOTE for the
    // appliance's roc10 migration: its entrypoint translated "=0" to UNSET
    // to disable -- that convention must switch to the _DISABLE var.
...
static constexpr __device__ vec_dot_q_cuda_t get_vec_dot_q_cuda(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_TQ1_0:   return vec_dot_tq1_0_q8_1;
        ...
        // ROC8 formats. F8E4M3 pairs with NATIVE e4m3 activations (the host
        // quantizes src1 with quantize_row_f8e4m3_for_mmvq_cuda for this type,
        // see ggml_cuda_mul_mat_vec_q below) so both dot operands feed
        // v_dot4_f32_fp8_fp8 directly; the others use lossless int8-activation
        // SIMD dispatch. The former f8e5m2 bf8-activation runtime toggle
...
static constexpr __host__ __device__ int get_vdr_mmvq(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_TQ1_0:   return VDR_TQ1_0_Q8_1_MMVQ;
...
bool ggml_cuda_should_use_mmvq(enum ggml_type type, int cc, int64_t ne11) {
    ...
    // ROC8 intercept-only types: IU4 decode rides its dedicated dot8 GEMV
    // (mmvq_iu4.cu, n==1 intercept in ggml-cuda.cu) and the 2:4-sparse types
    // ride SWMMAC intercepts -- none of them have a generic mmvq entry, so
    // they must never be routed here.
    if (type == GGML_TYPE_IU4 || type == GGML_TYPE_2OF4_FP8 || type == GGML_TYPE_2OF4_F16) {
        return false;
    }
    // ROC8 fp8/MX decode kernels: the vec_dots themselves are portable
    // (software e4m3/e5m2/e8m0 decode where hardware dots are absent), but
    // these tensor types are only produced/loaded for gfx1201 today -- keep
    // other archs on the dequant fallback until validated there too.
    if (type == GGML_TYPE_F8E4M3 || type == GGML_TYPE_F8E5M2 ||
        type == GGML_TYPE_MXFP8  || type == GGML_TYPE_MXFP6) {
...
static constexpr __host__ __device__ int calc_rows_per_block(ggml_type type, int ncols_dst, int table_id, bool small_k = false, int nwarps = 1) {
    ...
    if (table_id == MMVQ_PARAMETERS_RDNA4) {
        // Decode (ncols_dst==1) rows-per-block, swept per type on gfx1201:
...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t ids_stride, const uint32_t nrows_dst, cudaStream_t stream) {
...
static void mul_mat_vec_q_switch_type(
    ...
    switch (type_x) {
        ...
        case GGML_TYPE_MXFP6:
            ...
            break;
        default:
            GGML_ABORT("mmvq switch_type: unhandled type %s", ggml_type_name(type_x));
            break;
    }
}
