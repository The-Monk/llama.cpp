...
void ggml_cuda_mul_mat_q(
...

    {
        ...
        if (use_native_fp4) {
            ...
            quantize_mmq_f8e5m2_cuda(src1_d, ids_src1.get(), src1_q8_1.get(), src0->type, ne10, s11, s12, s13,
                                   ne10_padded, ne11_flat, ne12_flat, ne13_flat, stream);
        } else {
...
bool ggml_cuda_should_use_mmq(enum ggml_type type, int cc, int64_t ne11, int64_t n_experts) {
    ...
    // T97: F8E5M2 native bf8 WMMA -- same RDNA4-only gate, no dp4a fallback
    // (vec_dot_f8e5m2_f8e5m2_dp4a, mmq.cuh), same decode/prefill split as
    // F8E4M3 (should_use_mmvq(F8E5M2) already routes ne11<=8 to the T77
