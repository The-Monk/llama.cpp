// See mul_mat_iu4_hipblaslt.cuh for the full rationale.
// IU4 prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY IU4-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_iu4_hipblaslt.cuh"
...
constexpr int   QKK   = QK_IU4;           // 32 elements per IU4 block
...
__device__ __forceinline__ float iuk_half2float(ggml_half h) {
    return __half2float(*reinterpret_cast<const __half *>(&h));
}

// IU4 dequant: 32 signed int4 values packed 2-per-byte (low nibble = elem 2b,
// high nibble = elem 2b+1) x one fp16 per-block scale d. Signed two's-complement
// 4-bit (0x8=-8 .. 0x7=+7). w = int4(qs) * d.
__device__ __forceinline__ float iuk_dequant_elem(const block_iu4 * blk, int t, float d) {
    const uint8_t byte = blk->qs[t >> 1];
    const int     nib  = (t & 1) ? (byte >> 4) : (byte & 0x0F);
    const int     v    = (nib < 8) ? nib : (nib - 16);   // sign-extend 4-bit
    return (float)v * d;
}

// Requant a IU4 weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_iu4_to_int8_perchannel(
    ...
    const block_iu4 * row = (const block_iu4 *)(wdata + n * nb01);
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float   d    = iuk_half2float(row[b].d);
        const float   w    = iuk_dequant_elem(&row[b], t, d);
...
__global__ void k_requant_iu4_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float d = iuk_half2float(row[b].d);
        amax = fmaxf(amax, fabsf(iuk_dequant_elem(&row[b], t, d)));
    }
    __shared__ float sred[1024];
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        const int64_t b = l / QKK; const int t = (int)(l % QKK);
        const float d = iuk_half2float(row[b].d);
        out[l] = ggml_cuda_fp32_to_e4m3(iuk_dequant_elem(&row[b], t, d) * inv);
    }
}
...
// independent of the source quant, so IU4 and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_IU4_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_IU4_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_IU4_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_iu4_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_iu4_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_iu4_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    ...
    static const int64_t M_THRESH = [](){
        const char * e = getenv("GGML_HIP_IU4_HIPBLASLT_MTHRESH");
...
bool ggml_cuda_op_mul_mat_iu4_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    static const int mode = (getenv("GGML_HIP_IU4_HIPBLASLT_FP8") != nullptr) ? MODE_F8 : MODE_I8;
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_iu4_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_iu4_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_iu4_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_iu4_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
