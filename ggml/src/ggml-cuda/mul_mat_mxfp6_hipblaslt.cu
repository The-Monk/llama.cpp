// See mul_mat_mxfp6_hipblaslt.cuh for the full rationale.
// MXFP6 prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY MXFP6-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_mxfp6_hipblaslt.cuh"
...
constexpr int   QKK   = QK_MXFP6;           // 32 elements per MXFP6 block
...
__device__ __forceinline__ float mxk_half2float(ggml_half h) {
    return __half2float(*reinterpret_cast<const __half *>(&h));
}

// MXFP6 dequant: 32 OCP MX FP6 (E3M2) elements packed 4-per-3-bytes + one UE8M0
// block scale. Element t: group g=t/4, position r=t%4; unpack the 4 codes from
// qs[3g..3g+2], upconvert the 6-bit E3M2 code to its bit-exact e4m3 byte, then
// to fp32 x d. Matches dequantize_mxfp6 (dequantize.cuh:157).
__device__ __forceinline__ float mxk_dequant_elem(const block_mxfp6 * blk, int t, float d) {
    const int g = t >> 2;   // group of 4 codes (0..7)
    const int r = t & 3;    // position within group (0..3)
    uint8_t codes[4];
    mxfp6_unpack4(blk->qs + 3*g, codes);
    return ggml_cuda_e4m3_to_fp32(ggml_cuda_e3m2_to_e4m3(codes[r])) * d;
}

// Requant a MXFP6 weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_mxfp6_to_int8_perchannel(
    ...
    const block_mxfp6 * row = (const block_mxfp6 *)(wdata + n * nb01);
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float   d    = ggml_cuda_e8m0_to_fp32(row[b].e);
        const float   w    = mxk_dequant_elem(&row[b], t, d);
...
__global__ void k_requant_mxfp6_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float d = ggml_cuda_e8m0_to_fp32(row[b].e);
        amax = fmaxf(amax, fabsf(mxk_dequant_elem(&row[b], t, d)));
    }
    __shared__ float sred[1024];
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        const int64_t b = l / QKK; const int t = (int)(l % QKK);
        const float d = ggml_cuda_e8m0_to_fp32(row[b].e);
        out[l] = ggml_cuda_fp32_to_e4m3(mxk_dequant_elem(&row[b], t, d) * inv);
    }
}
...
// independent of the source quant, so MXFP6 and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_MXFP6_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_MXFP6_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_MXFP6_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_mxfp6_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_mxfp6_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_mxfp6_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    if (src0->type != GGML_TYPE_MXFP6)                       return false;
    ...
    static const int64_t M_THRESH = [](){
        const char * e = getenv("GGML_HIP_MXFP6_HIPBLASLT_MTHRESH");
...
bool ggml_cuda_op_mul_mat_mxfp6_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    GGML_ASSERT(src0->type == GGML_TYPE_MXFP6);
    ...
    static const int mode = (getenv("GGML_HIP_MXFP6_HIPBLASLT_FP8") != nullptr) ? MODE_F8 : MODE_I8;
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_mxfp6_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_mxfp6_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_mxfp6_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_mxfp6_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
