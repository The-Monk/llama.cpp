// See mul_mat_q4_K_hipblaslt.cuh for the full rationale.
// Q4_K prefill via hipBLASLt int8/fp8 GEMM. Structure mirrors the Q2_0 route
// (mul_mat_q2_0_hipblaslt.cu); the ONLY Q4_K-specific part is the dequant inside
// the two requant kernels -- everything else (activation quant, GEMM, plan/algo
// cache, scale application) is quant-agnostic.

#include "mul_mat_q4_K_hipblaslt.cuh"
...
constexpr int   QKK   = QK_K;           // 256 elements per Q4_K superblock
...
__device__ __forceinline__ float q4k_half2float(ggml_half h) {
    return __half2float(*reinterpret_cast<const __half *>(&h));
}

// Extract (d, dmin) from a Q4_K superblock. In the CUDA build the d/dmin fields
...
__device__ __forceinline__ float2 q4k_d_dmin(const block_q4_K * blk) {
    return __half22float2(*reinterpret_cast<const __half2 *>(&blk->dm));
}

// 6-bit scale/min unpack (device copy of get_scale_min_k4, ggml-quants.c:1354).
__device__ __forceinline__ void q4k_get_scale_min(int j, const uint8_t * scales, uint8_t & sc, uint8_t & mn) {
    if (j < 4) {
        sc = scales[j]   & 63;
        mn = scales[j+4] & 63;
    } else {
...
// Fully dequantize weight at position t (0..255) within a Q4_K superblock:
//   w = d*sc*q - dmin*m   (min folded in -> the returned value is the true weight)
// Layout (matches dequantize_row_q4_K): chunk c=t/64, sub-block sb=2c+(t%64>=32),
// nibble from qs[32c + (t%64)%32] (low if t%64<32 else high).
__device__ __forceinline__ float q4k_dequant_elem(const block_q4_K * blk, int t, float d, float dmin) {
    const int c  = t >> 6;                 // chunk 0..3
    const int p  = t & 63;                 // 0..63 within chunk
    const int sb = 2*c + ((p >= 32) ? 1 : 0);
    const int qi = c*32 + (p & 31);        // byte index into qs (0..127)
    const uint8_t qb  = blk->qs[qi];
    const int     nib = (p < 32) ? (qb & 0xF) : (qb >> 4);
    uint8_t sc, mn;
    q4k_get_scale_min(sb, blk->scales, sc, mn);
    return d * (float)sc * (float)nib - dmin * (float)mn;
}

// Requant a Q4_K weight row -> int8 with ONE symmetric scale per output channel.
// One block per output row n. Output int8 row-major [N x K] (== col-major [K x N]).
__global__ void k_requant_q4_K_to_int8_perchannel(
    ...
    const block_q4_K * row = (const block_q4_K *)(wdata + n * nb01);
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float2  dd   = q4k_d_dmin(&row[b]);
        const float   w    = q4k_dequant_elem(&row[b], t, dd.x, dd.y);
...
__global__ void k_requant_q4_K_to_e4m3_perchannel(
        const char * __restrict__ wdata, int64_t nb01,
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        ...
        const float2 dd = q4k_d_dmin(&row[b]);
        amax = fmaxf(amax, fabsf(q4k_dequant_elem(&row[b], t, dd.x, dd.y)));
    }
    __shared__ float sred[1024];
    ...
    for (int64_t l = threadIdx.x; l < K; l += blockDim.x) {
        const int64_t b = l / QKK; const int t = (int)(l % QKK);
        const float2 dd = q4k_d_dmin(&row[b]);
        out[l] = ggml_cuda_fp32_to_e4m3(q4k_dequant_elem(&row[b], t, dd.x, dd.y) * inv);
    }
}
...
// independent of the source quant, so Q4_K and Q2_0 share winners for a shape) --
#define TUNE_S2(x) #x
...
const char * tune_cache_path() {
    static std::string path = [](){
        if (const char * e = getenv("GGML_HIP_Q4_K_HIPBLASLT_TUNE_CACHE")) return std::string(e);
...
const lt_plan & get_plan(int64_t N, int64_t M, int64_t K, int mode) {
    ...
    if (built) {
        ...
        if (hipblasLtMatmulPreferenceCreate(&pref) == HIPBLAS_STATUS_SUCCESS &&
            ...
            nAlgo > 0) {
            static const bool notune = (getenv("GGML_HIP_Q4_K_HIPBLASLT_NOTUNE") != nullptr);
            int best = 0;
...
size_t wcache_budget_bytes() {
    static size_t b = [](){
        const char * e = getenv("GGML_HIP_Q4_K_HIPBLASLT_WCACHE_MB");
...
const cached_w * try_cache_weight(const void * key, const char * wdata, int64_t nb01,
    ...
    if (mode == 1 /*MODE_F8*/) {
        k_requant_q4_K_to_e4m3_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, (uint8_t *)c.q8, c.wscale, K, n_blocks);
    } else {
        k_requant_q4_K_to_int8_perchannel<<<grid, block, 0, stream>>>(wdata, nb01, c.q8, c.wscale, K, n_blocks);
    }
    g_wcache_bytes += need;
...
bool ggml_cuda_q4_K_hipblaslt_prefill_supports(const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * dst) {
    if (src0->type != GGML_TYPE_Q4_K)                       return false;
...
bool ggml_cuda_op_mul_mat_q4_K_hipblaslt(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    static const int mode = (getenv("GGML_HIP_Q4_K_HIPBLASLT_FP8") != nullptr) ? MODE_F8 : MODE_I8;
    ...
    if (cw) {
        ...
        const dim3 grid((unsigned)N), block(256);
        if (mode == MODE_F8) {
            k_requant_q4_K_to_e4m3_perchannel<<<grid, block, 0, stream>>>(
                (const char *)src0->data, src0->nb[1], (uint8_t *)wq8_ptr, wsc_ptr, K, n_blocks);
        } else {
            k_requant_q4_K_to_int8_perchannel<<<grid, block, 0, stream>>>(
...
bool ggml_cuda_q4_K_hipblaslt_prefill_supports(const ggml_tensor *, const ggml_tensor *, const ggml_tensor *) {
    return false;
}
bool ggml_cuda_op_mul_mat_q4_K_hipblaslt(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *) {
    return false;
}
