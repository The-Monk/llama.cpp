...
const std::map<std::string, common_speculative_type> common_speculative_type_from_name_map = {
    ...
    // DSpark is a DFlash-family block-diffusion drafter (see src/models/dspark.cpp);
    // it reuses the draft-dflash implementation unchanged (only the gguf metadata
    // key names differ, handled inside common_speculative_impl_draft_dflash).
    {"draft-dspark",  COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH},
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            for (int32_t i = 1; i < n_block_tokens; ++i) {
                ...
                if (getenv("DSPARK_DEBUG_DRAFT")) {
                    // dspark's own ctx_dft vocab is VOCAB_TYPE_NONE (dummy tokens only,
                    // no real special-token markers) -- check EOG against ctx_tgt's real
                    // vocab, which is what the shared token-id space actually refers to.
                    fprintf(stderr, "DSPARK_DEBUG_DRAFT: seq=%d n_block_tokens=%d i=%d id_last=%d beg=%d draft_id=%d p=%.4f is_eog=%d\n",
                            seq_id, n_block_tokens, i, dp.id_last, beg, id, cur_p->data[0].p,
                            (int) llama_vocab_is_eog(llama_model_get_vocab(llama_get_model(params.ctx_tgt)), id));
                }
